    ...
    for kitchen in config.COMBINATION_TO_COMPUTE['k']:
        for machine in config.COMBINATION_TO_COMPUTE[kitchen]:
            for arepas in config.COMBINATION_TO_COMPUTE[machine]:
                logging.info(f"[STEP]: Filtering dataframes for combination {kitchen}, {machine}, {arepas} "
                             f"between timestamps [START]: {config.START_TIMESTAMP} "
                             f"and [END]: {config.END_TIMESTAMP} - STARTED")
                br_df, cm_df, fi_df = utils.filter_datasets(config.START_TIMESTAMP,
                ...
                logging.info(f"[STEP]: Filtering dataframes for combination {kitchen}, {machine}, {arepas} "
                             f"between timestamps [START]: {config.START_TIMESTAMP} "
                             f"and [END]: {config.END_TIMESTAMP} - [RESULT]: COMPLETED")
                ...
                if not output_df.empty:
                    ...
                    logging.error(f"[STEP]: Writing output file - [RESULT]: ERROR - filtered too much data")
